...
export default class tabProfileSpecialist extends Component {
  state = { 
    ...
    PoliticaParticipacaoEspecialistaTexto: translate.val("PoliticaParticipacaoEspecialista"),
    TermosUsoTexto: translate.val("TermosUso"),
    SelecioneFrequencia: translate.val("SelecioneFrequencia"),
    EspecialistaColaborarIdentificacao: translate.val("EspecialistaColaborarIdentificacao"),
    ...
    DesejaRealmenteDeixarSerEspecialista: translate.val("DesejaRealmenteDeixarSerEspecialista"),
    NewSpecialist: false,
    CurrentSpecialist: false,
    NewValueSpecialist: false,
    TermosUsoLidoAceitoEspecialista: translate.val("TermosUsoLidoAceitoEspecialista"),
  ...
  getInfoUser()
  {
    ...
    .then((response) => { 
      ...
      if(!Functions.isNullOrEmpty(responseJson.alert_period))
      {
        ...
        {
          if(Functions.isEquals(period.key, responseJson.alert_period))
          {
          ...
          (_, results) => {
            ...
            for(let i = 0; i < results.rows.length; i++){
              ...
              groups.push({
                ...
                id:group.id,
                value: group.value,
                checked: expertises.findIndex(function(element){return element ==  group.id})>-1
              });              
            }
  ...
  selectCheckBoxPlant(id, index)
  {
    ...
    for(let i = 0; i < this.state.OptionsPlants.length; i++){
      array.push({
        ...
        checked: (i != index ? this.state.OptionsPlants[i].checked : !this.state.OptionsPlants[i].checked),
      });  
    }
  ...
  selectCheckBoxAnimal(id, index)
  {
    ...
    for(let i = 0; i < this.state.OptionsTaxgrp.length; i++){
      array.push({
        ...
        checked: (i != index ? this.state.OptionsTaxgrp[i].checked : !this.state.OptionsTaxgrp[i].checked),
      });  
    }     
  ...
  validateCurriculum()
  {
    if(Functions.isNullOrEmpty(this.state.Curriculum))
    {
      ...
      this.setState({inputCurriculumMessageError: this.state.CampoObrigatorio});
    }
    else
    {
  ...
  UpdateUser()
  {
    ...
    if(this.state.NewValueSpecialist == 0)
    {
      ...
      this.setState({inputAgreementMessageError: this.state.ErroAceitarTermosUso});     
      validFields = false;
    }
    else
    {
        ...
        if(responseJson.status == "ok")
        {
          ...
          conn.transaction(tx => {
            tx.executeSql(
              ...
              [this.state.NomeCompleto,this.state.Email, this.state.NomeUsuario, this.state.ImageSource.uri, this.state.DataNascimento, this.state.EscolaridadeLabel, this.state.GeneroLabel, this.state.IdiomaLabel,this.state.Password, true, this.state.AppCode], 
              (_, results) => 
              {
                this.setState({
                  isLoggedIn: true
  ...
  RemoverEspecialista()
  {
    Alert.alert(
      ...
      [
        ...
        {text: this.state.Sim, onPress: () => this.RemoverEspecialista_() }
      ]
    );
  }
      ...
      <Container>
        <Modal
          ...
          <Content  style={{flex: 1}}>
            <ScrollView>
              <Text style={styles.TitleTermsOfUse}>{this.state.PoliticaParticipacaoEspecialistaTexto}</Text>
              <Label style={styles.TextTermsOfUse} >
                ...
                Além disso, otimiza a geração de dados científicos para as análises por outros cientistas ou tomadores de decisão.
              </Label>
              <Text style={styles.TitleTermsOfUse}>Identificação dos espécimes nas fotos</Text>
              <Label style={styles.TextTermsOfUse}>
                É desejável a identificação a nível de espécie, porém é totalmente aceitável a identificação como família e gênero nos casos em que as fotos não permitirem 
                uma identificação específica.
                Sugerimos que as identificações sejam feitas no prazo máximo de 15 dias a partir da data de envio das fotos pelos guardiões.
                Você pode escolher com que frequência deseja receber notificações por e-mail de observações enviadas pelos guardiões que necessitam de identificação ou de validação. 
                Entre as opções estão: diariamente, semanalmente e mensalmente. Caso não deseje receber notificações por e-mail, basta escolher a opção "não quero receber notificações".
                Ao identificar os espécimes, você se compromete em fornecer os nomes corretos e aceitos atualmente.
                Não é necessário incluir o nome da espécie completo com o autor.
              </Label>
              
              <Text style={styles.TitleTermsOfUse}>Direitos autorais</Text>
              <Label style={styles.TextTermsOfUse}>
                Ao identificar um espécime, você concorda em receber os créditos pela identificação, tendo, assim, seu nome associado ao espécime identificado na foto.
                A identificação dos espécimes ficará visível para o guardião e para o público em geral que acessar o sistema.     
              </Label>
            </ScrollView>
          </Content>      
        </Modal>
        ...
        <Content padder>
          <Form>
            <Text style={styles.LabelInfo}>{this.state.EspecialistaColaborarIdentificacao}</Text>
            <View>
              <Item floatingLabel
                error={this.state.inputCurriculumError}
              >
                <Label style={styles.LabelDefault}>{this.state.EspecifiqueLinkCurriculoTexto}</Label>
                <Input
              ...
              </Item>
              {
                this.state.inputCurriculumError ?
                  <Label style={styles.LabelError}>{this.state.inputCurriculumMessageError}</Label>
                :
                  <Label></Label>
              }
            </View>
            ...
            <ListItem>
              <Text style={styles.LabelTitulo}>{this.state.PlantasTexto}</Text>
            </ListItem>
            <List
              ...
              renderRow={data => 
                <ListItem>
                  <CheckBox
                    ...
                    onPress={() => this.selectCheckBoxPlant(data.id, data.key)}
            ...
            />
            <ListItem>
              <Text style={styles.LabelTitulo}>{this.state.AnimaisTexto}</Text>
            </ListItem>
            <List
              ...
              renderRow={data => 
                <ListItem>
                  <CheckBox
                    color="#cc9933"
                    style={styles.checkBoxStyle}
                    onPress={() => this.selectCheckBoxAnimal(data.id, data.key)}
                    checked={data.checked}                  />
                  <Body>
                    <Text style={{color: "gray", fontSize:13}}
                      onPress={() => this.selectCheckBoxAnimal(data.id, data.key)}
            ...
            <View style={{ flex: 1, justifyContent: 'space-around', padding: 5 }}>
              <Label style={styles.LabelNotificacao}>{this.state.QueroReceberNotificacaoTexto}</Label>  
              <ModalSelector
                data={frequencyNotifications}
                cancelText="Cancelar"
                initValue= {this.state.NaoQueroReceberTexto}                      
                optionTextStyle={{color: "black"}}
                onChange={option => { this.setState({FrequencyNotifications:option.label, AlertPeriod: option.key })}}>   
            ...
            <View>
              ...
              >
                <Label style={styles.LabelDefault}>{this.state.ComentariosTexto}</Label>
                <Input
