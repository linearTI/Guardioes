...
class LoginView extends React.Component {
	...
	_GoogleSignin() {
		...
		GoogleSignin.signIn()
			...
			.catch((err) => {
				alert(
					this.state.ErroLoginNaoFoiPossivel + ":" + JSON.stringify(err)
				);
				...
				.then((response) => {
					...
					if (responseJson.status == "ok") {
						...
						Functions.trustFetch(Api, {
							...
							var conn = StorageManager.getConnection();
							conn.transaction((tx) => {
								tx.executeSql("UPDATE GuardianModel SET Picture = ?", [
									responseJson.picture,
								]);
							});
			...
			<Container style={styles.container}>
				...
				<Content>
					<View style={{ flex: 1 }}>
						...
						<Text style={styles.TextoOutroLogin}>
							{this.state.AcessarCom}
						</Text>
							...
							onLoginFinished={(error, result) => {
								if (error) {
									...
									AccessToken.getCurrentAccessToken().then((data) => {
										...
										fetch(
											"https://graph.facebook.com/me?fields=email,name,picture.type(large),friends&access_token=" +
												accessToken
										)
						...
						<Button
							style={[
								styles.buttonInstagram,
								styles.buttonCommonInstagram,
							]}
						...
						>
							<IconFontAwesome
								...
								size={20}
								color="white"
								style={stylesButtons.icon}
							>
								&nbsp;&nbsp;
								<Text style={stylesButtons.textoLogin}>
									{this.state.LoginInstagram}
								</Text>
							</IconFontAwesome>
						</Button>
