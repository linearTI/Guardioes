		...
		this.listenerLanguage = EventRegister.addEventListener(
			...
			(data) => {
				this.setState({
					...
					Escolaridades: translate.valEscolaridades(),
					IdiomaList: translate.valIdiomas(),
					TermosUsoLidoAceitoGuardiao: translate.val(
						"TermosUsoLidoAceitoGuardiao"
					),
				});
			}
		);
		...
		Functions.trustFetch(Api, {
			...
			.then((response) => {
				...
				for (let escolaridade of this.state.Escolaridades) {
					if (
						Functions.isEquals(escolaridade.key, responseJson.education)
					) {
				...
				this.setState({
					...
					DataTermosUsoLidoAceitoGuardiao: Moment(response.Date).format(
						"DD/MM/YYYY"
					),
				});
			...
			<Container>
				<Content padder>
					<Form>
						...
						<Label style={styles.LabelTermosUso}>
							{this.state.TermosUsoLidoAceitoGuardiao}{" "}
							{this.state.DataTermosUsoLidoAceitoGuardiao}
						</Label>
