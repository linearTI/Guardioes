...
const B = (props) => (
	<Text style={{ fontWeight: "bold", color: "#cc0033" }}>
		{props.children}
	</Text>
);
...
class NewContributionView extends React.Component {
	...
	componentWillMount() {
		...
		this.listenerLanguage = EventRegister.addEventListener(
			...
			(data) => {
				this.setState({
					...
					ErroMensagemPlanta: translate.val("ErroMensagemPlanta"),
					ErroMensagemBranco: translate.val("ErroMensagemBranco"),
					ErroMensagemLocalizacao: translate.val(
						"ErroMensagemLocalizacao"
					),
					ErroMensagem: translate.val("ErroMensagem"),
					SalvandoMensagem: translate.val("SalvandoMensagem"),
					ErroAoSalvar: translate.val("ErroAoSalvar"),
		...
		this.listenerReload = EventRegister.addEventListener("Reload", (data) => {
			...
			var newEventtime =
				Functions.pad(index, 2) + "" + Functions.pad(index + 2, 2);
			var newEventTimeName =
				Functions.pad(index, 2) +
				" h - " +
				Functions.pad(index + 2, 2) +
				" h";
	...
	componentDidMount() {
		...
		if (
			...
			this.setState({
				ID: id,
				visible: true,
				MessageLoading: this.state.CarregandoObservacao,
			});
				...
				tx.executeSql(
					...
					(tx, results) => {
						if (results.rows.length > 0) {
							...
							["A", "B", "C", "D", "E", "F", "G", "H"].map(function (
								letter,
								index
							) {
								if (
									!Functions.isNullOrEmpty(
										row["Photo_" + letter + "_Thumbnail"]
									)
								) {
									newState["Photo" + (index + 1) + "_Thumbnail"] = {
										uri:
							...
							newState["A_identificationremarks"] =
								row.A_identificationremarks;
							...
							newState["P_identificationremarks"] =
								row.P_identificationremarks;
							...
							newState["SistemaLocalizacao"] = translate.val(
								"SistemaLocalizacao"
							);
							newState["SelecioneOrigemFoto"] = translate.val(
								"SelecioneOrigemFoto"
							);
							newState["Camera"] = translate.val("Camera");
							newState["Galeria"] = translate.val("Galeria");
							newState["Cancelar"] = translate.val("Cancelar");
							newState["ErroMensagemIteracao"] = translate.val(
								"ErroMensagemIteracao"
							);
							newState["ErroMensagemPlanta"] = translate.val(
								"ErroMensagemPlanta"
							);
							newState["ErroMensagemBranco"] = translate.val(
								"ErroMensagemBranco"
							);
							newState["ErroMensagemLocalizacao"] = translate.val(
								"ErroMensagemLocalizacao"
							);
							newState["ErroMensagem"] = translate.val("ErroMensagem");
							newState["SalvandoMensagem"] = translate.val(
								"SalvandoMensagem"
							);
							newState["ErroAoSalvar"] = translate.val("ErroAoSalvar");
							newState["CarregandoObservacao"] = translate.val(
								"CarregandoObservacao"
							);
							newState["Observação"] = translate.val("Observação");
							newState["ObservacaoEnviada"] = translate.val(
								"ObservacaoEnviada"
							);
							newState["ErroAoCarregar"] = translate.val(
								"ErroAoCarregar"
							);
							newState["DesejaExcluir"] = translate.val("DesejaExcluir");
							newState["SelecionarGrupoAnimal"] = translate.val(
								"SelecionarGrupoAnimal"
							);
							newState["Selecionarlocalizacao"] = translate.val(
								"Selecionarlocalizacao"
							);
							newState["InstrucaoLocalizacao"] = translate.val(
								"InstrucaoLocalizacao"
							);
							newState["VisualizarFoto"] = translate.val(
								"VisualizarFoto"
							);
							newState["FotosInteracao"] = translate.val(
								"FotosInteracao"
							);
							newState["InstrucaoFotos"] = translate.val(
								"InstrucaoFotos"
							);
							newState["SelecionarFoto"] = translate.val(
								"SelecionarFoto"
							);
							newState["PerguntaAnimal"] = translate.val(
								"PerguntaAnimal"
							);
							...
							newState["ErroMensagemData"] = translate.val(
								"ErroMensagemData"
							);
							newState["ErroMensagemIntervalo"] = translate.val(
								"ErroMensagemIntervalo"
							);
							newState["LocalizacaoAtivado"] = translate.val(
								"LocalizacaoAtivado"
							);
							newState["AguardandoPrimeiraFoto"] = translate.val(
								"AguardandoPrimeiraFoto"
							);
							newState["SelecioneHabito"] = translate.val(
								"SelecioneHabito"
							);
							newState["SelecionarManualmente"] = translate.val(
							...
							newState["CompletarAnimal"] = translate.val(
								"CompletarAnimal"
							);
							...
							newState["InteracaoAnimalPlanta"] = translate.val(
								"InteracaoAnimalPlanta"
							);
							newState["SelecioneTipoInteracao"] = translate.val(
								"SelecioneTipoInteracao"
							);
							newState["DescrevaInformacoes"] = translate.val(
								"DescrevaInformacoes"
							);
							newState["ObservacoesGerais"] = translate.val(
								"ObservacoesGerais"
							);
							...
							newState["Animal"] = translate.val("Animal");
		...
		if (
			this.props.navigation.state.params != undefined &&
			this.props.navigation.state.params.welcome == 1
		) {
			var conn = StorageManager.getConnection();
			conn.transaction((tx) => {
				tx.executeSql(
	...
	initialSetup() {
		...
		conn.transaction((tx) => {
			...
			tx.executeSql("SELECT * FROM EventTime", [], (_, results) => {
				var times = new Array();
				for (let i = 0; i < results.rows.length; i++) {
			...
			if (response.didCancel) {
				...
				ImageResizer.createResizedImage(
					...
					.then((resizedImageUri) => {
						RNFetchBlob.fs
							.readFile(resizedImageUri.path, "base64")
							.then((dataImage) => {
								Thumbnail = dataImage;
								let source = { uri: resizedImageUri.uri };
								sourceFoto[
									"Photo" + selectedPhoto + "_Thumbnail"
								] = source;
				...
				if (
					!Functions.isNullOrEmpty(dateHour) &&
					!this.state.PhotoEventdate
				) {
					dateHour = Moment(response.timestamp).format(
						"DD/MM/YYYY HH:mm:ss"
					);
	...
	InserirFoto(index, photoData, Thumbnail) {
		...
		if (this.state.ID > 0) {
			...
			sqlcommand =
				"UPDATE ContributionModel SET " +
				"Sended = ?, " +
				"Provisional = ?, " +
				"Photo_" +
				photoLetters[index - 1] +
				" = ?,  " +
				" Photo_" +
				photoLetters[index - 1] +
				"_Thumbnail = ?,  " +
	...
	findAddress(latitude, longitude) {
		var link = Functions.linkApiLatLon(latitude, longitude);
		return Functions.trustGET(link)
			.then((response) => response.text())
			.then((text) => {
				var info = text.split(";");
				infoAddress = {
					Country: info.length > 0 ? info[0] : "",
					Stateprovince: info.length > 1 ? info[1] : "",
					Municipality: info.length > 2 ? info[2] : "",
					Locality:
						info.length > 3
							? info[2].trim().toUpperCase() ===
							  info[3].trim().toUpperCase()
								? ""
								: info[3]
							: "",
				};
	...
	SaveContribution() {
		...
		if (errorMessages.length > 0) {
			...
			this.setState({
				visible: true,
				MessageLoading: this.state.SalvandoMensagem,
			});
			var conn = StorageManager.getConnection();
			...
			<Container>
				...
				>
					<Header>
						...
						<Title style={styles.TitleNew}>
							{this.state.Selecionarlocalizacao}
						</Title>
						<Right>
							<Button
								transparent
								onPress={() => {
									...
									this.findAddress(
										this.state.modalMarker.latitude,
										this.state.modalMarker.longitude
									);
									this.setState({ modalMap: false });
								}}
							>
					...
					<Label style={styles.ObservacaoMapa}>
						<B>Obs:</B>
						{this.state.InstrucaoLocalizacao}
					</Label>
					<MapView
				...
				>
					<Header>
						...
						<Right>
							<Button
								transparent
								onPress={() => {
									Alert.alert("", this.state.DesejaExcluir, [
										{
											text: this.state.Nao,
											...
											onPress: () => {
												...
												if (this.state.modalPhotoIndex <= 4) {
													...
													if (this.state.modalPhotoIndex == 1) {
														...
														this.state.modalPhotoIndex == 2
													) {
														this.setState({
															Photo2_Thumbnail: none_1,
														});
													} else if (
														this.state.modalPhotoIndex == 3
													) {
														this.setState({
															Photo3_Thumbnail: none_1,
														});
													...
													cleanPhoto[
														"Photo" +
															this.state.modalPhotoIndex +
															"_Thumbnail"
													] = none_1;
													for (let i = 1; i <= 4; i++) {
														if (
															i != this.state.modalPhotoIndex &&
															this.state[
																"Photo" + i + "_Thumbnail"
															] != none_1
														) {
					...
					<Image
						source={this.state.modalPhoto_Source}
						style={{ flex: 1 }}
					/>
				</Modal>
				...
				>
					<Header>
						...
						<Body>
							<Title style={stylesSearch.title}>
								{this.state.BuscarEspecie}
							</Title>
						</Body>
						<Right>
							<Button
				...
				>
					...
					<ProgressiveInput
						...
						isLoading={this.state.visible}
						onChangeText={this._onChangeTextSearchFamilyAnimalia.bind(
							this
						)}
						style={{ marginTop: 5 }}
					/>
				...
				>
					...
					<ProgressiveInput
						...
						isLoading={this.state.visible}
						onChangeText={this._onChangeTextSearchSpeciesPlantae.bind(
							this
						)}
						style={{ marginTop: 5 }}
					/>
				...
				>
					...
					<ProgressiveInput
						...
						isLoading={this.state.visible}
						onChangeText={this._onChangeTextSearchFamilyPlantae.bind(
							this
						)}
						style={{ marginTop: 5 }}
					/>
					...
					<Form>
						...
						>
							<Row style={styles.sectionTitleRow}>
								<Text style={{ color: "green", fontSize: 20 }}>
									{this.state.FotosInteracao}
								</Text>
							</Row>
							<Row style={{ marginTop: 10 }}>
								<Text style={{ color: "gray" }}>
									{this.state.InstrucaoFotos}
								</Text>
							</Row>
							<Row style={{ marginTop: 10 }}>
								<Col style={{ alignItems: "flex-end" }}>
									<TouchableOpacity
										onPress={this.selectPhoto1}
										onLongPress={() => {
											if (this.state.Photo1_Thumbnail != "") {
												this.setState({
													modalPhoto_Source: this.state
														.Photo1_Thumbnail,
													modalPhoto: true,
													modalPhotoIndex: 1,
												});
									...
									>
										<View style={stylesCamera.ImageContainerAnimal}>
											{this.state.Photo1_Thumbnail === null ? (
												<Text>{this.state.SelecionarFoto}</Text>
											) : (
												<Image
													style={stylesCamera.ImageContainerAnimal}
													source={this.state.Photo1_Thumbnail}
												/>
											)}
										</View>
									</TouchableOpacity>
								...
								>
									...
									>
										<View style={stylesCamera.ImageContainerAnimal}>
											{this.state.Photo2_Thumbnail === null ? (
												<Text>{this.state.SelecionarFoto}</Text>
											) : (
												<Image
													style={stylesCamera.ImageContainerAnimal}
													source={this.state.Photo2_Thumbnail}
												/>
											)}
										</View>
									</TouchableOpacity>
							...
							<Row style={{ marginTop: 10 }}>
								<Col style={{ alignItems: "flex-end" }}>
									...
									>
										<View style={stylesCamera.ImageContainerAnimal}>
											{this.state.Photo3_Thumbnail === null ? (
												<Text>{this.state.SelecionarFoto}</Text>
											) : (
												<Image
													style={stylesCamera.ImageContainerAnimal}
													source={this.state.Photo3_Thumbnail}
												/>
											)}
										</View>
									</TouchableOpacity>
								...
								>
									...
									>
										<View style={stylesCamera.ImageContainerAnimal}>
											{this.state.Photo4_Thumbnail === null ? (
												<Text>{this.state.SelecionarFoto}</Text>
											) : (
												<Image
													style={stylesCamera.ImageContainerAnimal}
													source={this.state.Photo4_Thumbnail}
												/>
											)}
										</View>
									</TouchableOpacity>
										...
										>
											<Item>
												<Input
													...
													editable={false}
													placeholder={
														this.state.SelecionarGrupoAnimal
													}
							...
							<Row style={styles.sectionTitleRow}>
								...
								>
									{this.state.FotoPlanta}
								</Text>
							</Row>
							<Row style={{ marginTop: 10 }}>
								<Text style={{ color: "gray" }}>
									{this.state.DescricaoFoto}
								</Text>
							</Row>
							<Row style={{ marginTop: 10 }}>
								<Col style={{ alignItems: "flex-end" }}>
									<TouchableOpacity
										onPress={this.selectPhoto5}
										onLongPress={() => {
									...
									>
										<View style={stylesCamera.ImageContainer}>
											{this.state.Photo5_Thumbnail === null ? (
												<Text>{this.state.SelecionarFoto}</Text>
											) : (
												<Image
													style={stylesCamera.ImageContainer}
													source={this.state.Photo5_Thumbnail}
												/>
											)}
										</View>
									</TouchableOpacity>
								...
								>
									...
									>
										<View style={stylesCamera.ImageContainer}>
											{this.state.Photo6_Thumbnail === null ? (
												<Text>{this.state.SelecionarFoto}</Text>
											) : (
												<Image
													style={stylesCamera.ImageContainer}
													source={this.state.Photo6_Thumbnail}
												/>
											)}
										</View>
									</TouchableOpacity>
							...
							<Row style={{ marginTop: 10 }}>
								<Col style={{ alignItems: "flex-end" }}>
									...
									>
										<View style={stylesCamera.ImageContainer}>
											{this.state.Photo7_Thumbnail === null ? (
												<Text>{this.state.SelecionarFoto}</Text>
											) : (
												<Image
													style={stylesCamera.ImageContainer}
													source={this.state.Photo7_Thumbnail}
												/>
											)}
										</View>
									</TouchableOpacity>
								...
								>
									...
									>
										<View style={stylesCamera.ImageContainer}>
											{this.state.Photo8_Thumbnail === null ? (
												<Text>{this.state.SelecionarFoto}</Text>
											) : (
												<Image
													style={stylesCamera.ImageContainer}
													source={this.state.Photo8_Thumbnail}
												/>
											)}
										</View>
									</TouchableOpacity>
							...
							<Row style={{ marginTop: 30, alignContent: "center" }}>
								{this.state.foundPosition === false ? (
									<Text
										style={{
											height: "100%",
											width: "100%",
									...
									>
										{this.state.AguardandoPrimeiraFoto}
									</Text>
								) : (
								...
								<TextInput
									onChangeText={(text) => {
										var actualAddress = {
											...
											Stateprovince: this.state.address
												.Stateprovince,
						...
						<View>
							...
							<ModalSelector
								...
								onChange={(option) => {
									this.setState({
										EventTimeName: option.label,
										Eventtime: option.value,
									});
									if (!this.state.PhotoEventdate) {
										var verbatimeventdate =
											this.state.Eventdate + " " + option.value;
										this.setState({
											Verbatimeventdate: verbatimeventdate,
										});
									}
						...
						<View>
							<Item floatingLabel>
								...
								<Input
									...
									onFocus={() =>
										this.openModalSearch("dsSearchSpecieAnimalia")
									}
									value={this.state.A_scientificname}
								/>
							</Item>
						...
						<View>
							<Item floatingLabel>
								...
								<Input
									...
									onFocus={() =>
										this.openModalSearch("dsSearchFamilyAnimalia")
									}
									value={this.state.A_family}
								/>
							</Item>
						...
						<View>
							<Item floatingLabel>
								...
								<Input
									...
									onFocus={() =>
										this.openModalSearch("dsSearchSpeciePlantae")
									}
									value={this.state.P_scientificname}
								/>
							</Item>
						...
						<View>
							<Item floatingLabel>
								...
								<Input
									...
									onFocus={() =>
										this.openModalSearch("dsSearchFamilyPlantae")
									}
									value={this.state.P_family}
								/>
							</Item>
						...
						<View>
							...
							>
								{this.state.InteracaoAnimalPlanta}
							</Text>
							<ModalSelector
