...
class SideBar extends Component {
	...
	componentWillMount() {
		this.listener = EventRegister.addEventListener(
			"SetProfileImage",
			(data) => {
				let source = !Functions.isNullOrEmpty(data.image)
		...
		this.listenerRoute = EventRegister.addEventListener(
			"SetCurrentRoute",
			(data) => {
				...
				tx.executeSql("SELECT * FROM GuardianModel", [], (_, results) => {
					if (results.rows.length > 0) {
						...
						this.setState({
							...
							ImagemUsuario: !Functions.isNullOrEmpty(row.Picture)
								? { uri: "data:image/jpeg;base64," + row.Picture }
								: imageProfileDefault,
							AppCode: row.AppCode,
	...
	LogOut() {
		Alert.alert("", this.state.DesejaSair, [
			{
				text: this.state.Nao,
				...
				onPress: () => {
					...
					conn
						.transaction((tx) => {
							...
							tx.executeSql(
								"INSERT INTO GuardianModel (AppCode, Name, Email, IsAuthenticated,CompletedRegistration, Idioma) VALUES (?,?,?,?,?,?);",
			...
			<Container>
				...
				>
							...
							<Row>
								<Col style={{ alignItems: "flex-start" }}>
									<Image
										...
										source={this.state.ImagemUsuario}
									/>
								</Col>
					...
					<List
						...
						renderRow={(data) =>
							((data.route != "Profile" && data.route != "LogOut") ||
								this.state.IsAuthenticated ||
								!Functions.isNullOrEmpty(this.state.Network)) && (
								<ListItem
									button
									noBorder
									onPress={() => {
